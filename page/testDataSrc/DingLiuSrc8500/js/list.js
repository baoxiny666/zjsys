...
function func_refresh(deviceNum) {
	$('<div></div>').dialog({
        ...
        closed : false,
        cache : false,
        href : getContextPath() + '/page/testDataSrc/DingLiuSrc8500/refresh.html',
        modal : true,
        onClose : function() {
            $(this).dialog('destroy');
        },
            ...
            handler : function() {
           	 $.messager.progress({title:'请等待',msg:'保存数据中...'});
   				$('#modelForm').form('submit', {
   					url: getContextPath() + "/zjsys_testDataSrc/zjDingliuSrcAction_refreshData8500.action",
...
function func_flyJuzhi(){
	var test_stad=0;var sample_no="";var deviceNum="";
	var tesDate="";var rowId="";
	var rows = $('#datagrid').datagrid('getSelections');
	for ( var i = 0; i < rows.length; i++) {
		test_stad = test_stad + parseFloat(rows[i].test_stad);
	...
	if (sample_no == "") {
	 ...
	 $('<div></div>').dialog({
	        ...
	        closed : false,
	        cache : false,
	        href : getContextPath() + '/page/testDataSrc/DingLiuSrc8500/junzhi.html',
	        modal : true,
	        onClose : function() {
	            $(this).dialog('destroy');
	        },
	        onLoad:function(){
	    	 ...
	    	 $("#test_stad_edit").textbox('setValue',test_stad/rows.length);
