...
var GydeviceNum="'ZJ-ZX-040','ZJ-ZX-039'";
/**
 * 表头信息
 */
...
function func_upload(deviceNum){	 
	 $('<div></div>').dialog({
	        ...
	        closed : false,
	        cache : false,
	        href : getContextPath() + '/page/testDataSrc/ZjGongyefenxiSrc039_040/refresh.html',
	        modal : true,
	        onClose : function() {
	            $(this).dialog('destroy');
	        },
...
function func_fly(){
	...
	for(var i=0; i<rows.length; i++){
		...
		if(sampleno==""){sampleno=rows[i].sampleNum;
		}else{
			if(sampleno!=rows[i].sampleNum){
				alert("请选择同一个样品下的数据发送！");return false;}
		}	
   ...
   for(var j=0;j<hfaArry.length;j++){
	...
	var fcad=(100-Mad-Aad-Vad); var vdvdaf=0;	
	 ...
	 $('<div></div>').dialog({
	        ...
	        closed : false,
	        cache : false,
	        href : getContextPath() + '/page/testDataSrc/ZjGongyefenxiSrc039_040/manager.html',
	        modal : true,
	        onClose : function() {
	            $(this).dialog('destroy');
	        },
