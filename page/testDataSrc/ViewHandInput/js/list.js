...
function initDataGrid() {
	$('#datagrid').datagrid({
		toolbar : '#tb',
		url : getContextPath() + "/zjsys_testDataSrc/viewHandInputAction_list.action",
		collapsible : false,
		pagination : true,//分页
		rownumbers : true, //行号
		striped : true,    //各行换色
...
function func_add() {
	 $('<div></div>').dialog({
         ...
         closed : false,
         cache : false,
         href : getContextPath() + '/page/testDataSrc/ViewHandInput/manager.html',
         modal : true,
         onClose : function() {
             $(this).dialog('destroy');
         },
             ...
             handler : function() {
            	 $.messager.progress({title:'请等待',msg:'保存数据中...'});
    				$('#modelForm').form('submit', {
    					url: getContextPath() + "/zjsys_testDataSrc/viewHandInputAction_add.action",
    					onSubmit:function(){
	 ...
	 $('<div></div>').dialog({
         ...
         buttons : [ {
             ...
             handler : function() {
            	 $.messager.progress({title:'请等待',msg:'保存数据中...'});
    				$('#modelForm').form('submit', {
    					url: getContextPath() + "/zjsys_testDataSrc/viewHandInputAction_edit.action",
    					onSubmit:function(){
    						var isValid = $(this).form('enableValidation').form('validate');
    						if (!isValid){
    							$.messager.progress('close');
...
function func_del(){
	...
	$.messager.confirm('删除', '确定删除数据么?', function(r){
		if (r){
			...
			$.ajax( {
				url : getContextPath() + "/zjsys_testDataSrc/viewHandInputAction_del.action",
				type : "post",
				dataType : "json",
