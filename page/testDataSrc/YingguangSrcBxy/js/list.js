...
$(function() {
 ...
 var GDDeviceNum=yinggytype.substring(0,yinggytype.indexOf("?"));
/**
 * 表头信息
 */
var head = [ [ {
...
function func_search(){
 ...
 function func_upload(){	
	 $('<div></div>').dialog({
	        ...
	        closed : false,
	        cache : false,
	        href : getContextPath() + '/page/testDataSrc/YingguangSrcBxy/refresh.html',
	        modal : true,
	        onClose : function() {
	            $(this).dialog('destroy');
	        },
	            ...
	            handler : function() {
	           	 $.messager.progress({title:'请等待',msg:'保存数据中...'});
	   				$('#modelForm').form('submit', {
	   					url : getContextPath() + "/zjsys_testDataSrc/yingguangSrcBxyAction_uploadYinguangData.action",				
...
function func_fly(){
	...
	$.messager.confirm('发送', '确定发送数据么?', function(r){
		if (r){
			...
			$.ajax( {
				url : getContextPath() + "/zjsys_testDataSrc/yingguangSrcBxyAction_fly.action",
				type : "post",
				dataType : "json",
...
function func_del(){
	...
	$.messager.confirm('删除', '确定删除数据么?', function(r){
		if (r){
			  $.messager.progress({title:'请等待',msg:'删除数据中...'});
			$.ajax( {
				url : getContextPath() + "/zjsys_testDataSrc/yingguangSrcBxyAction_del.action",
				type : "post",
				dataType : "json",
				data : "recordIdS=" + rowId,
				async : true,
...
function func_recovery(){
	...
	$.messager.confirm('恢复', '确定恢复数据么?', function(r){
		if (r){
			...
			$.ajax( {
				url : getContextPath() + "/zjsys_testDataSrc/yingguangSrcBxyAction_recovery.action",
				type : "post",
				dataType : "json",
				data : "recordIdS=" + rowId,
				async : true,
