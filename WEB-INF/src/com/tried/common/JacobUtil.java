...
public class JacobUtil {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	jacobUtil.openWord(false);
    	jacobUtil.openDocument("C:\\Users\\angel\\Desktop\\报告\\new.docx");
    	if(jacobUtil.find(jacobUtil.selection, "${qrcode}")){
    		String imgPath="C:\\Users\\angel\\Desktop\\报告\\reportpng.png";
    		jacobUtil.replaceImage(jacobUtil.selection,imgPath,null,null);
    	}
     ...
     * 添加二维码
      * @Description
      * @author liuxd
      * @date 2017-4-1 上午11:16:28
      * @version V1.0
     */
    public void copyQrcode(String targetFile,String targetCode, Map<String,Object> qrcodeInfoParam) throws Exception{
    	if(qrcodeInfoParam.get("qrcodeImage")==null||qrcodeInfoParam.get("qrcodeLeft")==null||qrcodeInfoParam.get("qrcodeTop")==null){
    		return;
    	}
    	JacobUtil jacobUtil=new JacobUtil();
    	jacobUtil.openWord(false);
    	jacobUtil.openDocument(targetFile);
    	if(jacobUtil.find(jacobUtil.selection, targetCode)){
    		jacobUtil.replaceImage(jacobUtil.selection,qrcodeInfoParam.get("qrcodeImage").toString(),Integer.valueOf(qrcodeInfoParam.get("qrcodeLeft").toString()),Integer.valueOf(qrcodeInfoParam.get("qrcodeTop").toString()));
    	}
    	jacobUtil.closeDocument();
    	jacobUtil.closeWord();
    }
        ...
        if (oldText.startsWith("table") || replaceObj instanceof List) {    
            ...
            if (oldText.indexOf("image") != -1    
                ...
                while (find(selection, oldText)) {    
                    replaceImage(selection, newText,null,null);    
                    Dispatch.call(selection, "MoveRight");    
                }    
            else    
                while (find(selection, oldText)) {    
     ...
     *            图片文件（全路径）  
     */    
    public void replaceImage(Dispatch selection, String imagePath,Integer left,Integer top) {  
