...
public class ExcelUtil {
            ...
            for (int k = firstCellNum; k <= lastCellNum; k++) {
               ...
               // if(cellValue.length() > 2 && cellValue.substring(0,2).equals("<%")) {
                if(cellValue.length() > 2 && cellValue.substring(0,2).equals("${")) {
                    ...
                    String key = cellValue.substring(3, cellValue.length()-2);
     ...
     * @param cellStyle 样式
     */
    public static void setValue(Sheet sheet, int row, int cell, Object value, float rowHeightInPoints,CellStyle cellStyle){
        Row rowIn = sheet.getRow(row);
        if(rowIn == null) {
            rowIn = sheet.createRow(row);
        }
        rowIn.setHeightInPoints(rowHeightInPoints);
    //    rowIn.setHeight(rowHeightInPoints);
        Cell cellIn = rowIn.getCell(cell);
        if(cellIn == null) {
            cellIn = rowIn.createCell(cell);
        }
     ...
     * @param cellStyle 样式
     */
    public static void setFloatValue(Sheet sheet, int row, int cell, Float value, CellStyle cellStyle){
        Row rowIn = sheet.getRow(row);
        if(rowIn == null) {
            rowIn = sheet.createRow(row);
        }
     ...
     * @param wb
     */
    public static void setRegionBorder(short border, CellRangeAddress region, Sheet sheet,Workbook wb){  
    ...
    public static void main(String[] args) throws Exception{
    	InputStream input = new FileInputStream("C:\\Users\\SUNLUNAN\\Desktop\\excel\\最终版.xlsx");//C:\\Users\\SUNLUNAN\\Desktop\\excel\\daochu1000.xlsx
		Workbook wb = null;
		wb = new XSSFWorkbook(input);
		Sheet sheet = wb.getSheetAt(0);
		for(int i=1;i<=sheet.getLastRowNum();i++){						
			System.out.println(i+"   "+ExcelUtil.getCellValue(sheet, i, 0)+"   "+ExcelUtil.getCellValue(sheet, i, 1)+"   "+ExcelUtil.getCellValue(sheet, i, 2)+"   "+ExcelUtil.getCellValue(sheet, i, 3)+"\n");
		}
    }
}
