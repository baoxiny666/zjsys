...
public class GongYeFXCircleThread039 implements Runnable{
	private static Logger logger = Logger.getLogger(GongYeFXCircleThread039.class);
	ApplicationContext springContext = ApplicationContextUtil.getApplicationContext();
	    //工业分析仪实时信息_挥发分FXX.DBF
	 ZjGongyefenxiSrcService zjGongyefenxiSrcService = (ZjGongyefenxiSrcService) springContext.getBean("zjGongyefenxiSrcServiceImpl");
		...
		public void run() {
			while(true){
				...
				try {
					//不为空的时候执行采集，否则无限循环
					if(ThreadStaticVariable.DataCircleMap.get("ZJ-ZX-039")!=null){
						circleTime=ThreadStaticVariable.DataCircleMap.get("ZJ-ZX-039").getCircleTimeNum()*1000;
						zjGongyefenxiSrcService.synCollect039("ZJ-ZX-039",null);
					}
