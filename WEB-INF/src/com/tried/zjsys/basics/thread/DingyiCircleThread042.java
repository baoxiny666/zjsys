...
public class DingyiCircleThread042 implements Runnable{
	private static Logger logger = Logger.getLogger(DingyiCircleThread042.class);
	...
	public void run() {
		while(true){
			...
			try {
				//不为空的时候执行采集，否则无限循环
				if(ThreadStaticVariable.DataCircleMap.get("ZJ-ZX-042")!=null){
					circleTime=ThreadStaticVariable.DataCircleMap.get("ZJ-ZX-042").getCircleTimeNum()*1000;
					zjDingliuSrcService.synCollect042("ZJ-ZX-042",null);
				}
