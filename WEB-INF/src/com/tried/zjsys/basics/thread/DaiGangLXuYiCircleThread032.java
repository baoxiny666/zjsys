...
public class DaiGangLXuYiCircleThread032 implements Runnable{
	private static Logger logger = Logger.getLogger(DaiGangLXuYiCircleThread032.class);
	...
	public void run() {
		while(true){
			...
			try {
				//不为空的时候执行采集，否则无限循环
				if(ThreadStaticVariable.DataCircleMap.get("ZJ-LX-032")!=null){
					circleTime=ThreadStaticVariable.DataCircleMap.get("ZJ-LX-032").getCircleTimeNum()*1000;
					zjDaiganglixueSrcService.synCollect032("ZJ-LX-032",null);
				}
				...
				logger.error("带钢力学仪ZJ-LX-032："+e.getMessage());
			}
		}
