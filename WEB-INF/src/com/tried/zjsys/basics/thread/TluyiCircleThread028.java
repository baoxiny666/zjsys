...
public class TluyiCircleThread028 implements Runnable{
	private static Logger logger = Logger.getLogger(TluyiCircleThread028.class);
	...
	public void run() {
		while(true){
			...
			try {
				//不为空的时候执行采集，否则无限循环
				if(ThreadStaticVariable.DataCircleMap.get("ZJ-ZX-028")!=null){
					circleTime=ThreadStaticVariable.DataCircleMap.get("ZJ-ZX-028").getCircleTimeNum()*1000;
					zjTanliuSrcService.synCollect("ZJ-ZX-028",null);
				}
