...
public class DaiGangLXuYiCircleThread034 implements Runnable{
	private static Logger logger = Logger.getLogger(DaiGangLXuYiCircleThread034.class);
	...
	public void run() {
		while(true){
			...
			try {
				//不为空的时候执行采集，否则无限循环
				if(ThreadStaticVariable.DataCircleMap.get("ZJ-LX-034")!=null){
					circleTime=ThreadStaticVariable.DataCircleMap.get("ZJ-LX-034").getCircleTimeNum()*1000;
					zjDaiganglixueSrcService.synCollect032("ZJ-LX-034",null);
				}
				...
				logger.error("带钢力学仪ZJ-LX-034："+e.getMessage());
			}
		}
