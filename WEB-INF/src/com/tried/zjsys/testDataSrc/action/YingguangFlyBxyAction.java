...
@Controller
@Scope("prototype")
public class YingguangFlyBxyAction extends BaseAction<YingguangFlyBxy> {
	private static Logger logger = Logger.getLogger(YingguangFlyBxyAction.class);  
	...
	public void list() {
		try {
			...
			outJsonData(yingguangFlyBxyService.findPage(new Page<YingguangFlyBxy>(page, rows), "from YingguangFlyBxy y, DataCircle c where y.circleId=c.id  " + this.condition).getResult());
		} catch (Exception e) {
	...
	public void add() {
		try {
			...
			yingguangFlyBxyService.add(model);
			outSuccessJson("添加成功");
		} catch (Exception e) {
			logger.error(e.getMessage());
			outErrorJson("添加失败");
	...
	public void del() {
		try {
			...
			yingguangFlyBxyService.beachDelete(ids);
			outSuccessJson("删除成功");
		} catch (Exception e) {
			logger.error(e.getMessage());
			outErrorJson("删除失败");
	...
	public void edit() {
		try {
			model.setRecordTime(new Date());
			model.setRecordUser(getCurrentUser().getId());
			yingguangFlyBxyService.update(model);
			outSuccessJson("修改成功");
		} catch (Exception e) {
			logger.error(e.getMessage());
			outErrorJson("修改失败");
